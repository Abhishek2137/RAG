...
def generate_answer(query: str, results):
    if not results:
        return "I could not find enough relevant information to answer that question."

    context_texts = []
    for _, chunk in results:
        text = getattr(chunk, "page_content", str(chunk))
        context_texts.append(" ".join(text.split()))

    best_text = context_texts[0]
    sentences = [s.strip() for s in best_text.split(".") if s.strip()]

    if not sentences:
        return best_text[:400] + ("..." if len(best_text) > 400 else "")
...
st.title("RAG Demo")
st.write("Ask a question and the app will retrieve relevant passages and generate a concise answer from them.")
